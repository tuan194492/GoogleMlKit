...
public class FaceMeshDetectorProcessor extends VisionProcessorBase<List<FaceMesh>> {
  ...
  protected void onSuccess(
    ...
    for (FaceMesh face : faces) {
      ...
      FaceMeshPoint rootPoint = face.getPoints(12).get(0);
      for (int i=1;i<=12;i++){
        int len = face.getPoints(i).size();
        faceMeshPoints.add(face.getPoints(i).get(0));
        faceMeshPoints.add(face.getPoints(i).get(len - 1));
      }
